...
const app = express();
const paciente = new cassandra.Client({
...
const receta = new cassandra.Client({
    contactPoints: ['cassandra-node1', 'cassandra-node2', 'cassandra-node3'],
    localDataCenter: 'datacenter1',
    authProvider: new cassandra.auth.PlainTextAuthProvider('cassandra', 'password123'),
    keyspace: 'rec'
...
async function create(req, res){
    try{
        const data = await paciente.execute('SELECT * FROM paciente WHERE rut = ? ALLOW FILTERING', [ req.body.rut ], { prepare: true });
        if(data.first() == null){
            ...
            await paciente.execute(query1, [id_paciente, req.body.nombre, req.body.apellido, req.body.rut, req.body.email, req.body.fecha_nacimiento ], { prepare: true });
            await receta.execute(query2, [id_receta, id_paciente, req.body.comentario, req.body.farmacos, req.body.doctor ], { prepare: true });
            ...
            await receta.execute(query1, [id_receta, data.first().id, req.body.comentario, req.body.farmacos, req.body.doctor ], { prepare: true });
...
async function edit(req, res){
    try{
        const data = await receta.execute('SELECT * FROM recetas WHERE id = ? ALLOW FILTERING', [ req.body.id ], { prepare: true });
        if(data.first() != null){
            const query1 = 'UPDATE recetas SET comentario = ?, farmacos = ?, doctor = ? WHERE id = ?';
            await receta.execute(query1, [req.body.comentario, req.body.farmacos, req.body.doctor, req.body.id], { prepare: true });
...
async function del(req, res){
    try{
        ...
        if(data.first() != null){
            ...
            await receta.execute(query1, [req.body.id ], { prepare: true });
...
async function pacientes(req, res){
    try{
        const data = await paciente.execute('SELECT * FROM paciente', [ ]);
        res.send(data.rows)
    } catch (err) {
        console.log(err);
    }
...
async function recetas(req, res){
    try{
        const data = await receta.execute('SELECT * FROM recetas', [ ]);
        res.send(data.rows)
    } catch (err) {
        console.log(err);
    }
